...
module.exports = async function (locals) {
    ...
    return renderFile(path.join(__dirname, 'templates/index.ejs'), {
        ...
        background: config.douban[type].background,
        classify: config.douban[type].classify,
        dblink: config.douban[type].dblink,
        text1: config.douban[type].text1,
        text2: config.douban[type].text2,
        text3: config.douban[type].text3,
        wish: data.wish,
